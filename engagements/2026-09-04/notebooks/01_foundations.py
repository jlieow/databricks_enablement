...
# MAGIC
# MAGIC This mirrors the landing / staging / reporting layers the health authority runs on a data warehouse
...
# MAGIC | Schema | `01_raw` | Landed clinical data, as-is |
# MAGIC | Schema | `02_bronze` | Combined streams, deduplicated |
# MAGIC | Schema | `03_silver` | Cleaned, validated, quality-flagged |
# MAGIC | Schema | `04_gold` | The consolidated encounter tables |
# MAGIC | Schema | `05_ops` | Models, cost, operational tables |
# MAGIC | Volume | `01_raw.landing` | Uploaded CSVs, one folder per district |
...
# MAGIC
# MAGIC Upload the seed files from `data/landing/<district>/` to
# MAGIC `/Volumes/enablement/01_raw/landing/<district>/`, one folder per health district:
# MAGIC
# MAGIC - `northmoor_district/`
# MAGIC - `eldervale_district/`
# MAGIC
# MAGIC In the UI: **Catalog > enablement > 01_raw > Volumes > landing**, then **Upload**, creating a
# MAGIC folder per district. The next cell creates the folders so the upload target exists.

# COMMAND ----------

for district in ["northmoor_district", "eldervale_district"]:
    path = f"/Volumes/{CATALOG}/01_raw/landing/{district}"
...
for district in ["northmoor_district", "eldervale_district"]:
    ...
    try:
        ...
        print(f"{district}: {len(files)} file(s)")
        for f in files:
            print(f"    {f.name:55s} {f.size:>10,} bytes")
    except Exception:
        print(f"{district}: folder empty or not found yet")
...
spark.sql(f"CREATE OR REPLACE TABLE {DEMO} AS SELECT 1 AS encounter_id, 'first write' AS note")
...
# MAGIC
# MAGIC - **Notebook 02** lands the first district's CSVs into raw with Auto Loader.
